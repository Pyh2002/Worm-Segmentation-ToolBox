...
if __name__ == '__main__':
    ...
    NNetwork.load_state_dict(torch.load('/best.pth'))  # Load the state dictionary from training
    NNetwork.eval()  # Put the model in evaluation mode
    #  Input frames transforms to match input from training
    transform = transforms.Compose([
        transforms.ToPILImage(),  # Make sure the starting point for the input data is a PIL image
        #transforms.Resize((1024, 1024)),  # Change the input image size if needed
        transforms.ToTensor()  # Convert to a tensor
    ])
    
    video_path = "/videos"  # Directory for the input videos
        ...
        if file_name.endswith('.avi'):
            ...
            while cap.isOpened():
                ...
                if success:  # Make sure video opened correctly
                    frame = cv2.cvtColor(frame, cv2.COLOR_BGR2GRAY)  # Convert the frame to greyscale
                    #  Apply transforms to the frame and send it to the GPU
                    input_tensor = transform(frame).unsqueeze(0).to(torch.device('cuda'))
                    #  Make sure that the model is not modified while getting predictions
                    with torch.no_grad():
                        output = NNetwork(input_tensor)  # Feed input tensor into model and get output prediction
                    ...
                    masked_frame = cv2.bitwise_and(frame, frame, mask=predicted_mask)
            ...
            cap.release()
            result.release()
            print(f"{file_name} Complete")  # Print to console when a video is complete
